...
public final class M1 extends AbstractAnalyzer {
    ...
    protected void execTemplate(AnalyzerRequest request, AnalyzerResponse response) {
       ...
       for(Col c : numericCols) {
           ...
           final Number min = table.eval(Min.from(c.getIndex()));
           final Number mean = table.eval(Mean.from(c.getIndex()));
           final Number range = table.eval(Range.from(c.getIndex()));
           final Number count = table.eval(Count.from(c.getIndex()));
           final Number max_occurs = table.eval(CountIf.from(c.getIndex(), (val) -> val.equals(max)));
           final Number min_occurs = table.eval(CountIf.from(c.getIndex(), (val) -> val.equals(min)));
